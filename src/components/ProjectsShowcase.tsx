...
const ProjectCard: React.FC<{ project: ProjectDetail }> = ({ project }) => {
  return (
    <div className="group relative bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 hover:border-purple-300 cursor-pointer h-full">
      ...
      <div className="relative p-6 h-full flex flex-col">
        <div className="flex-grow">
          ...
          <p className="text-purple-500 text-sm font-medium mb-3">{project.duration}</p>
          {project.role && (
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 group-hover:border-purple-100 transition-colors duration-300">
          <div className="flex flex-wrap gap-1">
            {project.technologies?.slice(0, 3).map((tech, index) => (
              <span 
                key={index} 
                className="bg-purple-50 text-purple-700 text-xs px-2 py-1 rounded-md group-hover:bg-purple-100 transition-colors duration-300"
              >
                {tech}
              </span>
            ))}
          ...
          <div className="mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {project.technologies.map((tech, index) => (
                <span 
                  key={index} 
                  className="bg-gradient-to-br from-purple-50 to-purple-100 text-purple-800 text-sm px-4 py-2 rounded-lg shadow-sm border border-purple-100"
                >
                  {tech}
                </span>
              ))}

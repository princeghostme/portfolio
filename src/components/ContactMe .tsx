const ContactMe = () => {
    return (
        <div className="w-full bg-teal-50 py-10 px-4 sm:px-6 border-t border-teal-100">
            <div className="max-w-6xl mx-auto">
                {/* Header */}
                <div className="text-center mb-8">
                    <h2 className="text-2xl md:text-3xl font-semibold text-teal-900 mb-2">Get In Touch</h2>
                    <div className="w-16 h-1.5 bg-teal-500 mx-auto rounded-full mb-4"></div>
                    <p className="text-base text-teal-700 max-w-2xl mx-auto">
...
const ContactCard = ({ icon, title, content, href }: any) => {
    return (
        <div className="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition duration-200 text-center">
            ...
            <h3 className="text-sm font-medium text-teal-800 mb-1">{title}</h3>
            <a
                href={href}
                target={title === "LinkedIn" ? "_blank" : "_self"}
                rel="noopener noreferrer"
                className="text-teal-600 hover:text-teal-700 text-sm break-words"
            >
                {content}
            </a>
        </div>

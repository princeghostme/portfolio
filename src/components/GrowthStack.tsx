...
const GrowthStack = () => {
  ...
  return (
    <div className="mx-auto w-full px-4 py-10">
      ...
      <div className="relative h-3 bg-rose-100 rounded-full mb-12">
        {/* Progress Line */}
        {growthData.map((_, index) => {
          ...
          return (
            <div
              key={`progress-${index}`}
              className="absolute h-3 bg-rose-500 rounded-full transition-all duration-700 ease-in-out"
              style={{ width, left: "87%" }}
            ></div>
          );
        })}
        ...
        {growthData.map((_, index) => {
          ...
          return (
            <div
              key={`marker-${index}`}
              className="absolute -top-1.5 w-5 h-5 bg-white border-4 border-rose-600 rounded-full shadow"
        ...
        {growthData.map((item, index) => {
          ...
          return (
            <div
              key={`year-${index}`}
              className="absolute top-6 text-[11px] text-center font-medium text-rose-700 w-20"
              style={{ left: position, transform: "translateX(-50%)" }}
            >
              {item.year}
            </div>
